...
>
> **⚠️ Dimensões viraram multi-valor (spec 47 §3.6, fonte):** cada dimensão (seção, categoria, membro, instituição, tipo de tabela) passou de **um valor** para uma **lista** de valores, com semântica de interseção — **E entre dimensões, OU dentro de uma dimensão** — armazenada em arrays `String[]` (sem FK; ids órfãos filtrados na leitura). Os campos escalares descritos abaixo (`sectionId`, `categoryId`, `memberUserId`, `institutionId`, `tableTypeId`) e as combinações proibidas (agora avaliadas por array não-vazio) devem ser lidos nessa chave. A regra multi-valor, o backfill (escalar→array de 1) e a retrocompat do backup são definidos na **spec 47 §3.6** — esta seção descreve o modelo escalar original por referência histórica. Vale **só para Orçamento**; metas de acúmulo (`Goal`, spec 47) seguem single-value.
>
> **⚠️ Dimensão "Membro" → "Responsável" (spec 47 §3.6.1, fonte):** a dimensão de gasto por pessoa filtra agora por **`responsiblePartyId`** (persona/party — `personal`/`group`/`external`) via `responsiblePartyIdsForFilter`, **não** por usuário. O texto abaixo que fala em `responsibleUserId`/`AccountMember.userId` (§2, §4 "Cálculo de progresso", §6 join SQL) está **defasado**: `Transaction.responsibleUserId` não existe mais; a atribuição é por party (ver spec 47 §3.1). O campo persistido segue `memberUserIds`, mas passa a guardar **`partyId`** (sem migração). Orçamento antigo com `userId` só casa se houver link party↔user resolvível pelo fallback de legado; senão, casa vazio até o usuário reabrir e reescolher o Responsável.
>
> **⚠️ Superfície `/planning/budgets` = config agnóstica de mês (spec 47 §5.9, fonte):** a aba Orçamento **não** é presa ao mês fiscal atual — é uma **página de configuração** (CRUD dos orçamentos) com uma **faixa de histórico por mês** por orçamento (status ok/alerta/estourou por mês, reusando os limiares deste doc). O **progresso ao vivo do mês** permanece exatamente onde este doc o define — **§4 "Exibição no dashboard mensal"** e **"Página de resumo do mês"** (widgets `budgets`/`kpi-budget-health`, intactos). O `monthId` usado nessas leituras por mês tolera cuid **e** uuid (contas com `Month.id` legado uuid — spec 47 §5.9.1).
