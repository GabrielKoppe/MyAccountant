...
- **Editor**: detectar match **só quando a descrição for editada nesta sessão** (flag `descriptionDirty`), nunca no mount — senão o ícone acende ao abrir a linha para editar outro campo.
...
- QUANDO abro uma transação já categorizada para editar **outro** campo (sem tocar na descrição), ENTÃO o ícone de apelido **NÃO DEVE** acender.
...
- **Criar responsável/categoria a partir do form do apelido** que ainda não exista — o form reusa os seletores existentes; criação inline de `ResponsibleParty` não passa por `OptionsContext` hoje e não será adicionada nesta spec.
