...
Adicionar `loading.tsx` com skeletons (componentes `Skeleton` do MUI, respeitando tokens do design system) nas rotas de maior custo: `months/[monthId]`, `dashboards/yearly/[year]`, `dashboards/monthly/[monthId]` e grupo `settings/`. Nas páginas de mês e dashboards, quebrar o conteúdo pesado em componentes async envolvidos em `<Suspense>` com fallback de skeleton, permitindo streaming do shell antes dos dados.

### 2.2 Revalidação granular (PERF-02)

Substituir `revalidatePath(\`/${ctx.accountId}\`, "layout")` nas actions de transação por revalidação das rotas realmente afetadas: a página do mês (`/${accountId}/months/${monthId}`) e o dashboard mensal correspondente. O `monthId` já está disponível no input/contexto de cada action.
...
- Calcular totais por tabela com `prisma.transaction.groupBy({ by: ["tableId"], _sum: { amountCents: true } })` dentro do `Promise.all` existente, eliminando o `reduce` em JS.
...
- Extrair a montagem de dados da página para `getMonthPageData()` (ver Referências Técnicas), reduzindo o `page.tsx` a composição.
...
Carregar todos os componentes de gráfico (recharts e nivo) via `dynamic(() => import(...), { ssr: false, loading: <Skeleton/> })`, seguindo o padrão já usado pelo SankeyChart. Centralizar os wrappers dinâmicos em `src/components/dashboards/charts/lazy.ts` para evitar repetição.
...
Criar `skills/performance/SKILL.md` documentando os padrões consolidados por esta spec, no mínimo: (a) quando criar `loading.tsx` vs `<Suspense>` inline; (b) regra de revalidação granular (nunca `"layout"` para mutações de dados de página); (c) `React.cache()` obrigatório em helpers de query compartilhados; (d) `dynamic()` para libs de visualização; (e) agregação no Postgres antes de agregação em JS; (f) checklist de performance para review de feature. Como parte da spec, envolver as funções exportadas de `src/lib/queries/*.ts` em `React.cache()`.
...
**PERF-01:**
- QUANDO o usuário navega para `months/[monthId]`, `dashboards/yearly/[year]` ou `dashboards/monthly/[monthId]`, a UI DEVE exibir um skeleton (via `loading.tsx` ou fallback de `Suspense`) antes dos dados chegarem.
- Os skeletons DEVEM usar componentes MUI com tokens do tema (sem hex hardcoded) e aproximar o layout final (sem layout shift perceptível ao carregar).

**PERF-02:**
- QUANDO qualquer action de `src/actions/transactions.ts` completa com sucesso, ela NÃO DEVE chamar `revalidatePath` com escopo `"layout"`.
...
- O arquivo `months/[monthId]/page.tsx` DEVE ficar com menos de 150 linhas após a extração de `getMonthPageData()`.
...
- Todas as funções exportadas de `src/lib/queries/*.ts` DEVEM estar envolvidas em `React.cache()`.
- SE a mesma função de query for chamada duas vezes na mesma renderização, o Prisma DEVE executar apenas uma query (verificável com log de queries em dev).
...
|---|---|---|
| `loading.tsx` vs `<Suspense>` | `loading.tsx` para o shell da rota + `<Suspense>` para blocos pesados internos | `loading.tsx` dá feedback de navegação imediato; `Suspense` permite streaming parcial sem esperar a query mais lenta |
| Revalidação | `revalidatePath` granular por rota (sem tag-based caching) | `revalidateTag` exigiria adotar `unstable_cache`/tags em todas as queries; granularidade por rota resolve o problema atual com mudança mínima |
| Dedupe de queries | `React.cache()` por render | Dedupe automático por request no RSC, sem introduzir camada de cache com invalidação própria |
...
|---|---|
| PERF-01 | `src/app/(app)/[accountId]/months/[monthId]/loading.tsx` (novo), `src/app/(app)/[accountId]/dashboards/yearly/[year]/loading.tsx` (novo), `src/app/(app)/[accountId]/dashboards/monthly/[monthId]/loading.tsx` (novo), `src/app/(app)/[accountId]/settings/loading.tsx` (novo), `page.tsx` das rotas citadas (Suspense) |
| PERF-02 | `src/actions/transactions.ts` |
| PERF-03, PERF-04 | `src/app/(app)/[accountId]/months/[monthId]/page.tsx`, `src/lib/serializers/transaction.ts` (novo), `src/lib/queries/month-page.ts` (novo, `getMonthPageData`) |
| PERF-05 | `src/components/transactions/TransactionRow.tsx`, `src/components/transactions/TransactionTable.tsx` |
| PERF-06 | `src/components/dashboards/charts/lazy.ts` (novo), componentes de gráfico em `src/components/dashboards/` |
| PERF-07 | `src/components/ui/NotificationBell.tsx` |
| PERF-08 | `skills/performance/SKILL.md` (novo), `src/lib/queries/*.ts` |

### Revalidação granular (PERF-02)

```ts
// ✅ Correto — revalida só o que a mutação afeta
export const createTransactionAction = defineAction(..., async (input, ctx) => {
  const tx = await createTransaction(ctx.accountId, input);
  revalidatePath(`/${ctx.accountId}/months/${input.monthId}`);
  revalidatePath(`/${ctx.accountId}/dashboards/monthly/${input.monthId}`);
  return tx;
});

// ❌ Anti-padrão — invalida AppBar + todas as subpáginas da Account
...
```ts
// ✅ Correto — uma ida ao banco, agregação no Postgres
const [sectionTotalsRaw, prevTotalsRaw, tableTotalsRaw] = await Promise.all([
  getSectionTotals(accountId, monthId, sectionIds),
  prevMonthItem ? getSectionTotals(accountId, prevMonthItem.id, sectionIds) : null,
...
```ts
// ✅ Correto — src/lib/queries/dashboards.ts
...
```ts
// ✅ Correto — src/components/dashboards/charts/lazy.ts
