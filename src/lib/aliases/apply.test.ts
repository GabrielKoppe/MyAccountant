...
import {
  type SuggestionApplicableFields,
...
function baseCurrent(overrides: Partial<SuggestionApplicableFields> = {}): SuggestionApplicableFields {
  return {
    description: null,
    notes: null,
    amountCents: "0",
...
// ─── Aplicação de apelido (só apelido — a Regra foi fundida no Apelido) ────────
describe("computeSuggestion — apelido", () => {
  it("alias=null não gera patch/changes", () => {
    const { patch, changes } = computeSuggestion(null, baseCurrent(), SOURCES);
    expect(patch).toEqual({});
    expect(changes).toEqual([]);
  });

  it("não gera patch/changes quando o apelido não define nada", () => {
    const { patch, changes } = computeSuggestion(baseAlias(), baseCurrent(), SOURCES);
    expect(patch).toEqual({});
    expect(changes).toEqual([]);
  });
  ...
  it("não gera change quando o valor do apelido já é igual ao atual", () => {
    ...
    const { patch, changes } = computeSuggestion(alias, current, SOURCES);
  ...
  it("descrição e notas: define patch e change quando diferem", () => {
    ...
    const { patch, changes } = computeSuggestion(alias, baseCurrent(), SOURCES);
  ...
  it("amountCents: aplica verbatim, sem inversão de sinal (DD-20)", () => {
    ...
    const { patch, changes } = computeSuggestion(
      alias,
  ...
  it("categoria (DD-18): troca de categoria preserva subcategoria explícita do apelido", () => {
    ...
    const current = baseCurrent({ categoryId: "cat-conta", subcategoryId: "sub-gas" });
    const { patch } = computeSuggestion(alias, current, SOURCES);
    expect(patch.categoryId).toBe("cat-lazer");
  ...
  it("institutionId/responsiblePartyId: define patch quando diferem", () => {
    ...
    const { patch } = computeSuggestion(alias, baseCurrent(), SOURCES);
    expect(patch.institutionId).toBe("inst-1");
    expect(patch.responsiblePartyId).toBe("party-1");
  });
});

describe("suggestionPatchToUpdateInput (aplicação em modo visualização — DD-23)", () => {
  it("patch vazio → input vazio", () => {
    expect(suggestionPatchToUpdateInput({})).toEqual({});
  });

  it("amountCents (string no TransactionRow) vira BigInt", () => {
    const out = suggestionPatchToUpdateInput({ amountCents: "12345" });
  ...
  it("passa os demais campos verbatim, incluindo null", () => {
    ...
    expect(suggestionPatchToUpdateInput(patch)).toEqual({
      description: "Sistema de Gás",
      notes: null,
      categoryId: "cat-conta",
      subcategoryId: null,
  ...
  it("só inclui as chaves presentes (patch parcial, DD-04)", () => {
    const out = suggestionPatchToUpdateInput({ categoryId: "cat-conta" });
