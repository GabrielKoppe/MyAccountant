...
export function FinanceTableCard({
  ...
  function handleSaveAsModel() {
    ...
    startTransition(async () => {
      ...
      enqueueSnackbar(`Modelo "${result.data.name}" salvo com ${result.data._count.items} item(ns).`, { variant: "success" });
    ...
    <Paper variant="outlined" sx={{ mb: 2, overflow: "hidden" }}>
      ...
      >
        ...
        <Box sx={{ flex: 1, display: "flex", alignItems: "center", gap: 1, overflow: "hidden" }}>
          ...
          {isReadOnly && <Chip label="somente leitura" size="small" color="warning" variant="outlined" />}
        </Box>

        <Typography fontWeight="bold" sx={{ minWidth: 120, textAlign: "right" }}>
        ...
        {!isReadOnly && (
          <>
            <Button
              ...
              variant="outlined"
              startIcon={<AddIcon />}
              onClick={() => { setCollapsed(false); setShowNewRow(true); }}
              disabled={isPending}
            >
      ...
      <Menu anchorEl={menuAnchor} open={!!menuAnchor} onClose={() => setMenuAnchor(null)}>
        ...
        <MenuItem onClick={() => { setMenuAnchor(null); handleToggleCount(); }}>
          <ListItemIcon>
            <Switch size="small" checked={table.countInMonth} readOnly tabIndex={-1} />
          </ListItemIcon>
          {m.financeTables.countInMonthLabel}
        </MenuItem>
        <MenuItem onClick={() => { setMenuAnchor(null); setModelName(table.name); setSaveModelOpen(true); }}>
      ...
      <Dialog open={renameOpen} onClose={() => setRenameOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={handleRename}>{m.common.save}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={saveModelOpen} onClose={() => setSaveModelOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ pt: 1 }}>
          <TextField
            ...
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); handleSaveAsModel(); } }}
          />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setSaveModelOpen(false)}>{m.common.cancel}</Button>
          <Button variant="contained" onClick={handleSaveAsModel} disabled={isPending || !modelName.trim()}>
            Salvar modelo
          </Button>
        </DialogActions>
      </Dialog>
