...
export function DailyHeatmap({ year, month, dailyTotals, onDayClick }: Props) {
  ...
  const dayMap = useMemo(() => {
    const m = new Map<number, DayTotal>();
    for (const d of dailyTotals) m.set(d.day, d);
    return m;
  }, [dailyTotals]);
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", mb: 0.75 }}>
        {WEEKDAY_LABELS.map((l, i) => (
          <Typography
            ...
            variant="caption"
            align="center"
            sx={{ fontSize: "0.6rem", color: "text.disabled", fontWeight: 500, display: "block" }}
          >
            {l}
          </Typography>
        ))}
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "3px" }}>
        {cells.map((day, i) => {
          if (!day) {
            return <Box key={`blank-${i}`} sx={{ aspectRatio: "1" }} />;
          }
          ...
          // Texto legível sobre o fundo: claro sobre células escuras, apagado sobre células vazias
          const textColor =
            heatIdx >= 3
              ? colors.text.inverse
              : heatIdx >= 1
