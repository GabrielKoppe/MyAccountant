...
import { isSignupAllowed } from "@/server/services/auth-service";
...
export const { auth, handlers, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async signIn({ user, account }) {
      // Bloquear Google OAuth se o email não estiver na allowlist E não tiver convite pendente
      if (account?.provider === "google" && user.email && !(await isSignupAllowed(user.email))) {
